		...
		if((lwip_rec_buf[0]==0xa8)&&(lwip_rec_buf[1]==(u8)flash_data[ID_ADDR])&&(lwip_rec_buf[7]==0x78))
		{
			...
			switch(keyword)
			{
				...
				case 0x06:
					...
					break;
				case 0x07:
					flash_data[IP_ADDR] = (u32) ((data16_2 & 0xffff)<<16)|data16;
					if(((u8)flash_data[IP_ADDR]) > 1)
					{
...
//						lwip_setup(ip4_add);
				...
				case 0x11:
					...
					flash_data[TRI_H_1_ADDR] = (u32) tri_h[0] + aver_ch[0];
					flash_data[TRI_L_1_ADDR] = (u32) tri_l[0] + aver_ch[0];
					qspi_init(XPAR_XQSPIPS_0_DEVICE_ID);
					qspi_write(PARA_ADDR);
					para_update();
					clr_rec_cnt();
				...
				case 0x12:
					...
					flash_data[TRI_H_2_ADDR] = (u32) tri_h[1] + aver_ch[1];
					flash_data[TRI_L_2_ADDR] = (u32) tri_l[1] + aver_ch[1];
					qspi_init(XPAR_XQSPIPS_0_DEVICE_ID);
					qspi_write(PARA_ADDR);
					para_update();
					clr_rec_cnt();
				...
				case 0x13:
					...
					tri_l[2] = data16_2;
					clr_rec_cnt();
					flash_data[TRI_H_3_ADDR] = (u32) tri_h[2] + aver_ch[2];
					flash_data[TRI_L_3_ADDR] = (u32) tri_l[2] + aver_ch[2];
					qspi_init(XPAR_XQSPIPS_0_DEVICE_ID);
					qspi_write(PARA_ADDR);
					para_update();
					clr_rec_cnt();
		...
		if((switch_rec_flag)&&(!first_packet_flag))
		{
			if(rec_buf1_cnt < sample_quantity)
			{
				...
				if((receive_buf1[(rec_buf1_cnt*SEND_SIZE)]==0xa5)&&(receive_buf1[(rec_buf1_cnt*SEND_SIZE+1)]==0xa5))
				{
					...
					receive_buf1[rec_buf1_cnt*SEND_SIZE+6] = ip4_seg;
		...
		else if((!switch_rec_flag)&&(!first_packet_flag))
		{
			if(rec_buf2_cnt < sample_quantity)
			{
				...
				if((receive_buf2[(rec_buf2_cnt*SEND_SIZE)]==0xa5)&&(receive_buf2[(rec_buf2_cnt*SEND_SIZE+1)]==0xa5))
				{
					...
					receive_buf2[rec_buf2_cnt*SEND_SIZE+6] = ip4_seg;
...
void send_flashdata(void)
{
	...
	data[15] = (u8)(flash_data[IP_ADDR]>>16);  //ip4_seg
...
void lwip_server_setup(u8 ip, u8 ip_seg)
{
	struct ip_addr ipaddr, netmask, gw;
	...
	IP4_ADDR(&ipaddr,  192, 168,   ip_seg, ip);
	IP4_ADDR(&netmask, 255, 255, 255,  0);
	IP4_ADDR(&gw,      192, 168,   ip_seg,  1);

...
static  XGpio xgpio_trig_h_ch1;
static  XGpio xgpio_trig_l_ch1;
static  XGpio xgpio_trig_h_ch2;
static  XGpio xgpio_trig_l_ch2;
static  XGpio xgpio_trig_h_ch3;
static  XGpio xgpio_trig_l_ch3;
...
//	int i=0;
	...
	Status = XGpio_Initialize(&xgpio_trig_h_ch1, 	GPIO_ID_TRIG_H_CH1);	if (Status != XST_SUCCESS) {return XST_FAILURE;	}
	Status = XGpio_Initialize(&xgpio_trig_l_ch1, 	GPIO_ID_TRIG_L_CH1);	if (Status != XST_SUCCESS) {return XST_FAILURE;	}
	Status = XGpio_Initialize(&xgpio_trig_h_ch2, 	GPIO_ID_TRIG_H_CH2);	if (Status != XST_SUCCESS) {return XST_FAILURE;	}
	Status = XGpio_Initialize(&xgpio_trig_l_ch2, 	GPIO_ID_TRIG_L_CH2);	if (Status != XST_SUCCESS) {return XST_FAILURE;	}
	Status = XGpio_Initialize(&xgpio_trig_h_ch3, 	GPIO_ID_TRIG_H_CH3);	if (Status != XST_SUCCESS) {return XST_FAILURE;	}
	Status = XGpio_Initialize(&xgpio_trig_l_ch3, 	GPIO_ID_TRIG_L_CH3);	if (Status != XST_SUCCESS) {return XST_FAILURE;	}
	...
	XGpio_SetDataDirection(&xgpio_trig_h_ch1, 	 1, 	GPIO_DIR_OUTPUT);
	XGpio_SetDataDirection(&xgpio_trig_l_ch1,  	 1, 	GPIO_DIR_OUTPUT);
	XGpio_SetDataDirection(&xgpio_trig_h_ch2, 	 1, 	GPIO_DIR_OUTPUT);
	XGpio_SetDataDirection(&xgpio_trig_l_ch2,  	 1, 	GPIO_DIR_OUTPUT);
	XGpio_SetDataDirection(&xgpio_trig_h_ch3, 	 1, 	GPIO_DIR_OUTPUT);
	XGpio_SetDataDirection(&xgpio_trig_l_ch3,  	 1, 	GPIO_DIR_OUTPUT);
	...
	gpio_set_trig_h_ch1		(0);         	//触发电平
	gpio_set_trig_l_ch1		(0);        	//触发电平
	gpio_set_trig_h_ch2		(0);         	//触发电平
	gpio_set_trig_l_ch2		(0);         	//触发电平
	gpio_set_trig_h_ch3		(0);         	//触发电平
	gpio_set_trig_l_ch3		(0);         	//触发电平
	gpio_set_pretrig		(50);          //预触发
...
void gpio_set_trig_h_ch1	(u16 dat){	XGpio_DiscreteWrite(&xgpio_trig_h_ch1, 	1, dat);	 }
void gpio_set_trig_l_ch1	(u16 dat){	XGpio_DiscreteWrite(&xgpio_trig_l_ch1, 	1, dat);	 }
void gpio_set_trig_h_ch2	(u16 dat){	XGpio_DiscreteWrite(&xgpio_trig_h_ch2, 	1, dat);	 }
void gpio_set_trig_l_ch2	(u16 dat){	XGpio_DiscreteWrite(&xgpio_trig_l_ch2, 	1, dat);	 }
void gpio_set_trig_h_ch3	(u16 dat){	XGpio_DiscreteWrite(&xgpio_trig_h_ch3, 	1, dat);	 }
void gpio_set_trig_l_ch3	(u16 dat){	XGpio_DiscreteWrite(&xgpio_trig_l_ch3, 	1, dat);	 }

...
u32 qspi_read(u32 Address)
{
	...
	for(i = 0;i < 64;i++)
	{
		flash_data[i] = ((u32)ReadBuffer[i*4+3]<<24)|((u32)ReadBuffer[i*4 + 2]<<16)|((u32)ReadBuffer[i*4 + 1]<<8)|((u32)ReadBuffer[i*4 + 0]);
	}
	if (Status != XST_SUCCESS) {
		return 0xffff;
	}
...
void qspi_write(u32 Address)
{
	int i=0;
	for (i = 0; i < 256; i++)
	{
		WriteBuffer[DATA_OFFSET + i] = (flash_data[(i/4)]>>((i%4)*8));
	}
	/*
	 * Erase the flash.
	 */
	FlashErase(&QspiInstance, Address, PAGE_SIZE);
...
void para_update(void)
{
	 ...
	 u8 ch = 0;
     qspi_read(PARA_ADDR);
     if(flash_data[SAM_PER_ADDR])
     {
    	 send_period = flash_data[SAM_PER_ADDR];
    	 sample_time = send_period*60*2;
     }
     ip_tmp = (u8) flash_data[IP_ADDR];
     ip4_seg = (u8) (flash_data[IP_ADDR]>>16);
     if(ip_tmp < 2)
     {
    	 ip4_add = 2;
     }
     ...
     gpio_set_trig_h_ch1		(((int)flash_data[TRI_H_1_ADDR]));         	//触发电平
	 gpio_set_trig_l_ch1		(((int)flash_data[TRI_L_1_ADDR]));        	//触发电平
	 gpio_set_trig_h_ch2		(((int)flash_data[TRI_H_2_ADDR]));         	//触发电平
	 gpio_set_trig_l_ch2		(((int)flash_data[TRI_L_2_ADDR]));          	//触发电平
	 gpio_set_trig_h_ch3		(((int)flash_data[TRI_H_3_ADDR]));          	//触发电平
	 gpio_set_trig_l_ch3		(((int)flash_data[TRI_L_3_ADDR]));        	//触发电平
}
